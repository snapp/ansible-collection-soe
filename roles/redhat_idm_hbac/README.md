# Ⓐ Ansible Role: redhat_idm_hbac

Manage Red Hat IdM Host-Based Access Control (HBAC).
...
- hosts: "{{ groups['idm_server'][0] }}"
  roles:
    - role: infra.soe.redhat_idm_hbac
      redhat_idm_hbac_rules:
        - name: allow_all
          description: Allow all commands
          users:
            - exampleuser
      tags: infra.soe.redhat_idm_hbac
```
